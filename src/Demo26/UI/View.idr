module Demo26.UI.View

import public Demo26.UI.Style
...
public export
data Exposed : Type -> Type where
  Expose : ((value -> View) -> View) -> Exposed value

namespace Exposed
  public export
  (>>=) : Exposed a -> (a -> View) -> View
  Expose f >>= g = f g


getIdentity : FC -> Elab String
getIdentity (MkFC (PhysicalIdrSrc (MkMI list)) (line, column) _) = pure "\{joinBy "/" $ reverse list}.idr:\{show line}:\{show column}"
getIdentity fc = pure $ show !(genSym "")
...
implementContextInterface identity state = declare [
    IClaim EmptyFC MW Public [Hint True] (MkTy EmptyFC EmptyFC (UN (Basic "Context(fromString \"\{identity}\")")) (IApp EmptyFC (IVar EmptyFC (UN (Basic "Context"))) (IPrimVal EmptyFC (Str identity)))),
    IDef EmptyFC (UN (Basic "Context(fromString \"\{identity}\")")) [
      PatClause EmptyFC
        (IVar EmptyFC (UN (Basic "Context(fromString \"\{identity}\")")))
        (IApp EmptyFC (
            INamedApp EmptyFC
              (IVar EmptyFC (NS (MkNS ["View", "UI", "Demo26"]) (UN (Basic "__mkContext"))))
              (UN (Basic "identity"))
